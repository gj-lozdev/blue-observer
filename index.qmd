            ...
            const popup = `
                <div style="display:flex; gap:10px; align-items:center; max-width:260px;">
                    <div>
                        ...
                        <small>${p.month}</small><br>

                        <br><strong>Météo moyenne :</strong><br>
                        🌡 Temp moyenne : ${p.avg_temp} °C<br>
                        🌧 Pluviométrie : ${p.avg_rain} mm/jour<br>
                        💨 Vent moyen : ${p.avg_wind} km/h<br>
                    </div>
                </div>
                `;
...
### {.sidebar}
```{ojs}
viewof species = Inputs.select(speciesList, {
  label: "Species",
  value: speciesList[1]   
})
